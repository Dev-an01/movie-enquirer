import argparse
import math
from lib.keyword_search import search_command
from lib.inverted_index import build_command as build_inverted_index
from lib.text_utils import tf_tokenize
from lib.inverted_index import InvertedIndex

BM25_K1 = 1.5
BM25_B = 0.75
def main() -> None:
    parser = argparse.ArgumentParser(description="Keyword Search CLI")
    subparsers = parser.add_subparsers(dest="command", help="Available commands")

    search_parser = subparsers.add_parser("search", help="Search movies using keywords")
    build_parser = subparsers.add_parser("build", help="Build the inverted index")
    tf_parser = subparsers.add_parser("tf", help="Get term frequency for a document and term")
    idf_parser = subparsers.add_parser("idf", help="Get inverse document frequency for a term")
    tfidf_parser = subparsers.add_parser("tfidf", help="Get TF-IDF score for a document and term")
    bm25idf_parser = subparsers.add_parser("bm25idf", help="Get BM25 IDF score for a term")
    
    
    search_parser.add_argument("query", type=str, help="Search query")
    tf_parser.add_argument("doc_id", type=int, help="Document ID")
    tf_parser.add_argument("term", type=str, help="Term to get frequency for")
    idf_parser.add_argument("term", type=str, help="Term to get inverse document frequency for")
    tfidf_parser.add_argument("doc_id", type=int, help="Document ID")
    tfidf_parser.add_argument("term", type=str, help="Term to get TF-IDF score for")
    bm25idf_parser.add_argument("term", type=str, help="Term to get BM25 IDF score for")
    
    
    # bm25_tf
    bm25_tf_parser = subparsers.add_parser(
        "bm25tf", help="Get BM25 TF score for a given document ID and term"
    )
    bm25_tf_parser.add_argument(
    "b", type=float, nargs="?", default=BM25_B, help="Tunable BM25 b parameter"
    )
    
    bm25_tf_parser.add_argument("doc_id", type=int, help="Document ID")
    bm25_tf_parser.add_argument("term", type=str, help="Term to get BM25 TF score for")
    bm25_tf_parser.add_argument(
        "k1", type=float, nargs="?", default=BM25_K1, help="Tunable BM25 K1 parameter"
    )
    bm25search_parser = subparsers.add_parser(
    "bm25search", help="Search movies using full BM25 scoring"
    )
    bm25search_parser.add_argument("query", type=str, help="Search query")
    args = parser.parse_args()
    
    
    match args.command:
        case "search":
            print("Searching for:", args.query)
            results = search_command(args.query)
            for i, res in enumerate(results, 1):
                print(f"{i}. {res['title']}")
        case "build":
            print("Building inverted index...")
            build_inverted_index()
        case "tf":
            # doc_id = int(input())
            # term = input()
            tf = tf_tokenize(args.term)
            index = InvertedIndex()
            index.load()
            term_frequency = index.get_tf(args.doc_id, tf)
            print(term_frequency)
        case "idf":
            term = tf_tokenize(args.term)
            index = InvertedIndex()
            index.load()
            print(index.get_idf(term))
        case "tfidf":
            term = tf_tokenize(args.term)
            index = InvertedIndex()
            index.load()
            print(index.get_tfidf(args.doc_id, term))
        case "bm25idf":
            term = tf_tokenize(args.term)
            index = InvertedIndex()
            index.load()
            print(index.get_bm25_idf(term))
        case "bm25tf":
            term = tf_tokenize(args.term)
            index = InvertedIndex()
            index.load()
            print(index.get_bm25_tf(args.doc_id, term))
        case "bm25search":
            print("Searching for:", args.query)
            index = InvertedIndex()
            index.load()
            results = index.bm25_search(args.query, limit=5)
            for i, (doc_id, score) in enumerate(results, 1):
                movie = index.movies[doc_id-1]
                print(f"{i}. {movie['title']} (Score: {score:.2f})")
                
        case _:
            parser.print_help()


if __name__ == "__main__":
    main()